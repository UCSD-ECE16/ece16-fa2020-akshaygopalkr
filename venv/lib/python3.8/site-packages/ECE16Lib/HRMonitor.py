  ...
  def remove_outliers(self, peaks):
    if len(peaks) > 1:
      ...
      for i in range(len(peak_diff)):
        ...
        # If the peak difference is more than 2 deviations above the average, add a peak (skipped)
        #elif peak_diff[i] > (avg_peak_diff + (2.5*std_peak_diff)):
          #peaks.insert(i+1, int((peaks[i]+peaks[i+1])/2))
